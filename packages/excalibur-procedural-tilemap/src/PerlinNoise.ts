...
/**
 * Initialize a new noise generator using the provided PRNG
 *
 * @param random a PRNG function like `Math.random` or `AleaPRNG.random`
 * @returns an initialized simplex noise generator
 */
export const Perlin = (random: PRNG) => {
  const tables = buildPermutationTables(random);
  return {
    noise2D: (x: number, y: number) => noise2D(tables, x, y),
    noise3D: (x: number, y: number, z: number) => noise3D(tables, x, y, z),
    noise4D: (x: number, y: number, z: number, w: number) =>
      noise4D(tables, x, y, z, w),
  };
};
